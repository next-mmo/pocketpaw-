  ...
  getMethods() {
    return {
      ...
      async loadMCData() {
        ...
        if (
          this.missionControl.agents.length > 0 &&
          !this.missionControl.loading
        ) {
          // Refresh activity feed and running-tasks in the background
          try {
            const [activityRes, runningRes] = await Promise.all([
              fetch("/api/mission-control/activity"),
              fetch("/api/mission-control/tasks/running"),
            ]);
            if (activityRes.ok) {
              const data = await activityRes.json();
              this.missionControl.activities = data.activities || [];
            }
            // Sync running tasks so stop/run buttons stay correct
            if (runningRes.ok) {
              const data = await runningRes.json();
              const serverRunning = data.running_tasks || [];
              const serverRunningIds = new Set(
                serverRunning.map((rt) => rt.task_id),
              );

              // Add newly running tasks
              for (const rt of serverRunning) {
                if (!this.missionControl.runningTasks[rt.task_id]) {
                  const agent = this.missionControl.agents.find((a) =>
                    rt.assignee_ids?.includes(a.id),
                  );
                  this.missionControl.runningTasks[rt.task_id] = {
                    agentId: rt.assignee_ids?.[0] || "",
              ...
              // Remove tasks that are no longer running on the server
              for (const taskId of Object.keys(
                this.missionControl.runningTasks,
              )) {
                if (!serverRunningIds.has(taskId)) {
        ...
        try {
          ...
          if (statsRes.ok) {
            ...
            this.missionControl.stats = {
              ...
              active_tasks:
                (raw.tasks?.by_status?.in_progress || 0) +
                (raw.tasks?.by_status?.assigned || 0),
          ...
          // Sync running tasks from backend so UI survives hard refresh.
          // Without this, runningTasks is empty after reload and tasks
          // that are still executing show as "in_progress" but with no
          // Stop button (isMCTaskRunning returns false).
          if (runningRes.ok) {
            const data = await runningRes.json();
            const serverRunning = data.running_tasks || [];
            for (const rt of serverRunning) {
      ...
      handleMCEvent(data) {
        ...
        if (eventType === "mc_task_started") {
          ...
          // Update agent status
          const agent = this.missionControl.agents.find(
            (a) => a.id === agentId,
          );
          if (agent) {
            agent.status = "active";
          ...
          if (
            this.missionControl.showAgentActivitySheet &&
          ...
          if (status === "completed") {
            ...
            this.missionControl.stats.active_tasks = Math.max(
              0,
              this.missionControl.stats.active_tasks - 1,
            );
          }
          ...
          if (this.missionControl.selectedProject && status === "completed") {
            fetch(
              `/api/deep-work/projects/${this.missionControl.selectedProject.id}/plan`,
            )
              .then((r) => (r.ok ? r.json() : null))
              .then((progData) => {
                if (progData) {
                  this.missionControl.projectProgress =
                    progData.progress || null;
                  this.missionControl.projectTasks =
                    progData.tasks || this.missionControl.projectTasks;
          ...
          if (status === "completed") {
            this.showToast(
              `Task completed: ${task?.title || taskId}`,
              "success",
            );
      ...
      handleDWEvent(data) {
        ...
        if (eventType === "dw_planning_phase") {
          ...
          const idx = this.missionControl.projects.findIndex(
            (p) => p.id === projectId,
          );
          if (idx >= 0) {
          ...
          this.log(
            `[Deep Work] Planning ${status}: ${title || projectId}`,
