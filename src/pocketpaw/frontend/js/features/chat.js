  ...
  getMethods() {
    return {
      ...
      handleNotification(data) {
        ...
        if (
          content.includes("Connected to PocketPaw") &&
      ...
      getTodoSlashAssist() {
        return {
          ...
          title: "Todo Copilot",
          subtitle:
            "Run Todo CRUD directly from chat with /todo add, /todo list, " +
            "/todo update, /todo done, and /todo delete.",
          summary: "Direct CRUD in chat",
          prompt_prefix: "/todo ",
          actions: [
            {
              key: "add",
              label: "Add a task",
              description:
                "Insert a new-task command template into the composer.",
              command: "/todo add ",
              behavior: "insert",
            },
            {
            ...
            },
            {
              ...
              label: "Update task",
              description:
                "Insert a template like /todo update 1 Rewrite landing page.",
              command: "/todo update ",
              behavior: "insert",
            },
            {
              key: "delete",
              label: "Delete task",
              description: "Insert a template like /todo delete 1.",
              command: "/todo delete ",
              behavior: "insert",
            },
          ],
        ...
        const commands = [
          {
            key: "todo",
            command: "/todo",
            description: "Open Todo Copilot for chat CRUD and helper actions.",
            kind: "App",
            insertText: "/todo ",
            priority: 0,
      ...
      getSlashSuggestions(limit = 8) {
        ...
        if (
          text.length > slashToken.length &&
          /\s/.test(text.charAt(slashToken.length))
        ) {
          return [];
        }
        ...
        return this.getSlashCommandCatalog()
          ...
          .sort((left, right) => {
            const leftExact =
              left.command.toLowerCase() === searchNeedle ? 0 : 1;
            const rightExact =
              right.command.toLowerCase() === searchNeedle ? 0 : 1;
            if (leftExact !== rightExact) return leftExact - rightExact;

            const leftPrefix = left.command
              .toLowerCase()
              .startsWith(searchNeedle)
              ? 0
              : 1;
            const rightPrefix = right.command
              .toLowerCase()
              .startsWith(searchNeedle)
              ? 0
              : 1;
      ...
      applySlashSuggestion(item = null) {
        const suggestions = this.getSlashSuggestions();
        const selected =
          item || suggestions[this.slashPicker.selectedIndex] || null;
        if (!selected) {
          this.hideSlashPicker();
          return false;
        }
      ...
      openComposerAssist(payload = {}) {
        const actions = Array.isArray(payload.actions)
          ? payload.actions
              .map((action, index) => ({
                ...
                behavior: String(action.behavior || "send")
                  .trim()
                  .toLowerCase(),
              }))
        ...
        this.composerAssist = {
          ...
          title:
            String(payload.title || "Ready in chat").trim() || "Ready in chat",
          subtitle: String(
            payload.subtitle || "Pick a quick action or type in the composer.",
          ).trim(),
          summary: String(payload.summary || "").trim(),
          promptPrefix: String(
            payload.prompt_prefix || payload.promptPrefix || "",
          ).trim(),
          actions,
          examples,
      ...
      syncComposerAssistWithInput() {
        ...
        const prefix = String(this.composerAssist.promptPrefix || "")
          .trim()
          .toLowerCase();
        if (!prefix) {
          return;
        ...
        const matchesPrefix =
          normalized === commandOnly || normalized.startsWith(prefix);
      ...
      buildChatMetadata(text, options = {}) {
        ...
        return {
          ...
          extension_chat_action:
            String(options.assistAction || "").trim() || null,
      ...
      runComposerAssistAction(action) {
        ...
        const behavior = String(action?.behavior || "send")
          .trim()
          .toLowerCase();
        if (behavior === "insert") {
          this.inputText = /\s$/.test(action.command)
            ? action.command
            : `${command} `;
          this.syncComposerAssistWithInput();
          this.hideSlashPicker();
      ...
      toggleAgent() {
        ...
        this.log(
          `Switched Agent Mode: ${this.agentActive ? "ON" : "OFF"}`,
          "info",
        );
      },
    };
