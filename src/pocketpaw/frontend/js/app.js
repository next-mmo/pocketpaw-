...
function app() {
  ...
  return {
    ...
    endpointVerifying: false,
    endpointVerifyResult: null,
    ...
    _startApp() {
      ...
      PocketPaw.EventBus.on("sidebar:files", (data) =>
        this.handleSidebarFiles(data),
      );
      PocketPaw.EventBus.on("output:files", (data) =>
    ...
    async restartServer() {
      if (
        !confirm("Restart the server? Active connections will be interrupted.")
      )
        return;
    ...
    getBackendDescription(backend) {
      const descriptions = {
        ...
        openai_agents:
          "OpenAI Agents SDK with code interpreter and file search. Works with OpenAI, Ollama, and local LLMs.",
        google_adk:
          "Google ADK — native Gemini agent with built-in tools: Google Search, code execution, MCP support.",
        opencode:
          "OpenCode AI coding agent. Requires server at configured URL.",
      };
    ...
    isCurrentBackendAvailable() {
      const b = this._backendsData.find(
        (x) => x.name === this.settings.agentBackend,
      );
    ...
    /**
     * Verify an OpenAI-compatible endpoint (ping /v1/models).
     */
    async verifyEndpoint() {
      const baseUrl = (this.settings.openaiCompatibleBaseUrl || "").trim();
      if (!baseUrl) {
        this.showToast("Enter a Base URL first", "error");
        return;
      }
      this.endpointVerifying = true;
      this.endpointVerifyResult = null;
      try {
        const resp = await fetch("/api/llm/verify", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            base_url: baseUrl,
            api_key: this.settings.openaiCompatibleApiKey || "",
          }),
        });
        const data = await resp.json();
        this.endpointVerifyResult = data;
        if (data.ok) {
          const modelCount = data.models_count || data.models?.length || 0;
          const latency = data.latency_ms ? ` (${data.latency_ms}ms)` : "";
          const msg =
            modelCount > 0
              ? `Connected${latency} — ${modelCount} model${modelCount !== 1 ? "s" : ""} available`
              : `Connected${latency}`;
          this.showToast(
            data.warning || msg,
        ...
        this.endpointVerifyResult = { ok: false, error: e.message };
        this.showToast("Verification failed: " + e.message, "error");
      } finally {
        this.endpointVerifying = false;
      }
    },
    ...
    backendNeedsKey(keyField) {
      ...
      if (backend === "claude_agent_sdk")
        provider = this.settings.claudeSdkProvider || "anthropic";
      else if (backend === "openai_agents")
        provider = this.settings.openaiAgentsProvider || "openai";
    ...
    getSettingsNavDot(sectionId) {
      if (sectionId === "general" || sectionId === "apikeys") {
        return this.isBackendUnconfigured()
          ? "bg-[var(--danger-color)]"
    ...
    _SETTINGS_INDEX: [
      {
        ...
        label: "Agent Backend",
        hint: "claude openai gemini codex opencode copilot",
      },
      {
        section: "general",
        sectionLabel: "General",
        label: "Provider",
        hint: "anthropic ollama openai-compatible",
      },
      {
        section: "general",
        sectionLabel: "General",
        label: "Model Override",
        hint: "claude sdk model",
      },
      {
        section: "general",
        sectionLabel: "General",
      ...
      {
        section: "behavior",
        sectionLabel: "Behavior & Safety",
        label: "Tool Profile",
        hint: "minimal coding full permissions",
      },
      {
        section: "behavior",
        sectionLabel: "Behavior & Safety",
        label: "Plan Mode",
        hint: "approval planning",
      },
      {
        section: "behavior",
        sectionLabel: "Behavior & Safety",
        label: "Injection Scanner",
        hint: "security prompt injection",
      },
      {
        section: "behavior",
        sectionLabel: "Behavior & Safety",
        label: "Smart Routing",
        hint: "model router simple complex",
      },
      {
        section: "memory",
        sectionLabel: "Memory",
      ...
      },
      {
        ...
        hint: "mem0 embedder openai ollama",
      },
      {
        section: "memory",
        sectionLabel: "Memory",
      ...
      },
      {
        ...
        sectionLabel: "Search & Services",
        label: "TTS Provider",
        hint: "voice openai elevenlabs sarvam",
      },
      {
        section: "services",
        sectionLabel: "Search & Services",
        label: "OCR Provider",
        hint: "vision tesseract",
      },
      {
        section: "system",
        sectionLabel: "System",
    ...
    searchSettings() {
      ...
      this.settingsSearchResults = this._SETTINGS_INDEX.filter(
        (item) =>
      ...
      const sections = [
        ...new Set(this.settingsSearchResults.map((r) => r.section)),
      ];
