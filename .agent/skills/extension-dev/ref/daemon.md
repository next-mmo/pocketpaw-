...
> **Important:** Both engines must serve the same API (e.g. OpenAI-compatible `/v1/chat/completions`). The frontend doesn't change behavior based on engine — only the backend command changes.
