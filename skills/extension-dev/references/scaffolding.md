...
When the user asks to create a new extension, **always scaffold a React + Vite project**. See [ui-stack.md](ui-stack.md) for the full UI setup and [manifest.md](manifest.md) for the `extension.json` format.
...
2. Create `extension.json` — see [manifest.md](manifest.md) for the field reference
...
2. Add `sandbox`, `install`, `start` configuration — see [sandbox.md](sandbox.md) and [daemon.md](daemon.md)
3. Create `requirements.txt` with Python dependencies
4. If CUDA is needed: add `--extra-index-url` to requirements.txt and/or `torch` config — see [sandbox.md](sandbox.md#pytorch-config-sandboxtorch-field)
5. If the plugin runs a server: add `start` config with `daemon: true`, `ready_pattern`, and `port: "auto"` — see [daemon.md](daemon.md#start-config-start-field)
6. For wrapping external repos: see [self-bootstrap.md](self-bootstrap.md)
