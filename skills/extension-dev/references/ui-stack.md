...
All PocketPaw extensions (SPA and Plugin) use **React + Vite + Ant Design** as the default UI framework. The Vite build outputs `index.html` and `assets/` into the extension root, which PocketPaw serves inside an iframe. See [scaffolding.md](scaffolding.md) for the step-by-step setup process.
...
Since extensions run inside an iframe, they must detect the parent PocketPaw origin. See [api.md](api.md) for the full list of available endpoints and [daemon.md](daemon.md#reverse-proxy) for how the reverse proxy works.
...
For very simple extensions that don't need React, you can use the PocketPaw SDK directly in a plain HTML file. See [manifest.md](manifest.md#available-scopes) for the scopes that control SDK features.
...
Every extension with a Vite build **must** gitignore build output and runtime dirs. See [sandbox.md](sandbox.md#shared-managed-locations) for what lives in each runtime directory.
