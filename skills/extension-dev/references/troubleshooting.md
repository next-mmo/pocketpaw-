...
Common issues and solutions. See [daemon.md](daemon.md) for process lifecycle details, [sandbox.md](sandbox.md) for runtime isolation, and [ui-stack.md](ui-stack.md) for build issues.

## General

- **Extension not showing**: Check `extension.json` is valid JSON and the `id` field matches the regex — see [manifest.md](manifest.md#field-reference)
- **Storage not working**: Ensure `storage.read`/`storage.write` are in scopes — see [manifest.md](manifest.md#available-scopes)
- **Chat errors**: Ensure `chat.send` or `chat.stream` scope is declared — see [chat-integration.md](chat-integration.md#required-scopes)
...
- **Install stuck**: Check `/api/v1/plugins/{id}/logs` for error messages — see [api.md](api.md#plugin-lifecycle)
- **CUDA not detected**: Ensure `nvidia-smi` is in PATH and NVIDIA drivers are installed — see [api.md](api.md#cuda-detection)
- **PyTorch wrong CUDA**: Check `GET /api/v1/plugins/cuda` for the auto-detected `cuda_tag`, then set `sandbox.torch.cuda` explicitly — see [sandbox.md](sandbox.md#supported-cuda-tags)
- **Port conflict**: Use `"port": "auto"` to let PocketPaw find a free port
- **Daemon won't start**: Check `ready_pattern` matches the stdout of your server — see [daemon.md](daemon.md#start-config-start-field)
- **CORS blocked**: Use the proxy endpoint (`/api/v1/plugins/{id}/proxy/...`) instead of direct `127.0.0.1:{port}` access — see [daemon.md](daemon.md#reverse-proxy)
...
See [daemon.md](daemon.md#process-lifecycle) for the full state machine.
...
See [ui-stack.md](ui-stack.md#vite-config) for the required Vite configuration.
