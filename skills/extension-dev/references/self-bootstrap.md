...
For extensions that wrap an existing open-source project (e.g. Gradio apps, Streamlit dashboards), use the **self-bootstrapping pattern**: clone the upstream repo at install time via `build.py`, then run its entry point via `start.path`. See [scaffolding.md](scaffolding.md) for the general extension setup.
...
The build script runs inside the [sandbox](sandbox.md) — `python`, `node`, `git`, etc. are all on PATH. See [sandbox.md](sandbox.md#install-steps-installsteps-field) for how the `run` step executes. For Windows-specific issues, see [troubleshooting.md](troubleshooting.md#self-bootstrapping-issues).
...
When wrapping a project that provides its own web UI, the extension can rely on PocketPaw's [reverse proxy](daemon.md#reverse-proxy) to embed the upstream UI directly:
